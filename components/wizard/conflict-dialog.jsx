...
export default function ConflictDialog({
  ...
  // 🆕 SESSION 47.16 — Cas "liste d'attente complète" (waitlist_full=true)
  const firstConflict = list[0] || {};
  const isWaitlistFull = firstConflict.waitlist_full === true;
  const waitlistMax = firstConflict.waitlist_max || 3;
  const title = isWaitlistFull
    ? `Liste d'attente complète`
    : isAnimation
      ? `${list.length} créneau${list.length > 1 ? 'x' : ''} déjà demandé${list.length > 1 ? 's' : ''}`
      : `Stand ${standCode || ''} ${firstConflict.owner_status === 'validated' ? 'déjà réservé' : 'pré-réservé'}`;
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-700 leading-relaxed">
            {isWaitlistFull ? (
              <>Ce stand a déjà atteint le nombre maximum de demandes en liste d&apos;attente (<b>{waitlistMax}</b> exposants). Veuillez sélectionner un autre stand.</>
            ) : isAnimation ? (
              <>Le{list.length > 1 ? 's' : ''} créneau{list.length > 1 ? 'x' : ''} sélectionné{list.length > 1 ? 's' : ''} {list.length > 1 ? 'sont' : 'est'} déjà demandé{list.length > 1 ? 's' : ''}. Votre demande peut être placée en <b>liste d&apos;attente</b> — ARACOM tranchera lors de la validation.</>
            ) : (
              <>Ce stand est déjà {firstConflict.owner_status === 'validated' ? 'réservé' : 'pré-réservé (en attente de validation)'}. Vous pouvez <b>soumettre votre demande d&apos;occupation de stand en liste d&apos;attente</b> — ARACOM tranchera lors de la validation finale.</>
            )}
          </DialogDescription>
        </DialogHeader>
            ...
            {list.map((c, idx) => {
              ...
              return (
                <div key={idx} className="border-2 border-amber-200 rounded-lg p-3 bg-amber-50/60 space-y-2">
                  ...
                  {/* 🆕 SESSION 47.16 — Anonymat : ne plus afficher le nom du propriétaire */}
                  <div className="text-sm flex items-center gap-2 flex-wrap">
                    <Users className="w-4 h-4 text-slate-500 shrink-0" />
                    <span className="text-slate-800">Statut actuel :</span>
                    <Badge variant="outline" className={isValidated ? 'border-emerald-400 text-emerald-700 bg-emerald-50' : 'border-amber-400 text-amber-700 bg-amber-50'}>
                      {isValidated ? '✅ Réservé' : '⏳ Pré-réservé (en attente)'}
                    </Badge>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <Clock className="w-4 h-4 text-slate-500 shrink-0" />
                    <span className="text-slate-700">
                      Liste d&apos;attente : <b>{c.waitlist_count || 0}/{c.waitlist_max || 3}</b> exposant{(c.waitlist_count || 0) > 1 ? 's' : ''}
                    </span>
                  </div>
        ...
        <DialogFooter className="gap-2 sm:gap-2 flex-col sm:flex-row">
          <Button variant="outline" onClick={onClose} disabled={submitting}>
            {isWaitlistFull ? 'Choisir un autre stand' : 'Annuler — choisir autre chose'}
          </Button>
          {!isWaitlistFull && (
            <Button
            ...
            >
              ...
              ⏳ Soumettre ma demande en liste d&apos;attente
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
