 ...
 */
async function anthropicChat({ model, system, user, history = [], max_tokens = 2000, temperature = 0.7 }) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return { ok: false, error: 'ANTHROPIC_API_KEY non configurée', skip: true };
  try {
    const messages = [];
    // Optional multi-turn history: [{role:'user'|'assistant', content}]
 ...
 */
async function emergentChatRaw({ model, system, user, history = [], max_tokens = 2000, temperature = 0.7 }) {
