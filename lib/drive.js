...
export function isDriveConfigured() {
  return Boolean(process.env.GOOGLE_SERVICE_ACCOUNT_FILE && process.env.GOOGLE_DRIVE_ROOT_FOLDER_ID);
}
...
export async function getDriveClient() {
  ...
  if (!file) throw new Error('GOOGLE_SERVICE_ACCOUNT_FILE non configuré');
  if (!fs.existsSync(file)) throw new Error('Fichier de credentials introuvable: ' + file);
  const credentials = JSON.parse(fs.readFileSync(file, 'utf-8'));
