...
async function computeBySite(db) {
  ...
  return venues.map(v => {
    ...
    // 🆕 SESSION 53.10 — confirmed inclut désormais verrouille + pre_validated (cohérence dashboard)
    const confirmed = vregs.filter(r => ['confirme', 'verrouille', 'pre_validated'].includes(r.status)).length;
