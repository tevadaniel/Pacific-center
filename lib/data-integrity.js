...
export async function runDataIntegrityCheck(db, { heal = true, verbose = false } = {}) {
  ...
  try {
    ...
    //   + 🛡️ Vérification cohérence : chaque token actif doit être résolvable
    // ════════════════════════════════════════════════════════════════
    for (const tk of tokens) {
      ...
      if (!orgValid && !userValid && !emailValid) {
        ...
        if (orgUsers && orgUsers.length > 0) {
          report.issues.push({ type: 'token_missing_user_id', token_id: tk.id, org_id: tk.organization_id });
          if (heal) {
      ...
      } else if (tk.user_id && !userValid) {
        // Token a user_id mais le user n'existe plus (a été supprimé)
        report.issues.push({ type: 'token_user_deleted', token_id: tk.id, deleted_user_id: tk.user_id });
        if (heal) {
          if (orgValid) {
            // Tente de relier au user actuel de l'org
            const orgUsers = userByOrg.get(tk.organization_id);
            if (orgUsers && orgUsers.length > 0) {
              await db.collection('access_tokens').updateOne({ id: tk.id }, { $set: { user_id: orgUsers[0].id, updated_at: new Date() } });
              report.healed.push({ type: 'token_relinked', token_id: tk.id, new_user_id: orgUsers[0].id });
            }
          } else {
            await db.collection('access_tokens').updateOne({ id: tk.id }, { $set: { revoked_at: new Date(), revoked_by: 'integrity_check', revoked_reason: 'user_deleted_no_org' } });
            report.healed.push({ type: 'token_revoked_user_deleted', token_id: tk.id });
          }
        }
      }

      // 🛡️ Test de résolution : simule le consume-token pour vérifier qu'il marche
      if (!orgValid && !userValid && !emailValid) continue; // déjà géré ci-dessus
      let resolvedUser = null;
      if (tk.user_id && userIds.has(tk.user_id)) {
        resolvedUser = users.find((u) => u.id === tk.user_id);
      }
      if (!resolvedUser && tk.email) {
        resolvedUser = userByEmail.get(tk.email.toLowerCase().trim());
      }
      if (!resolvedUser && tk.organization_id) {
        const orgUsers = userByOrg.get(tk.organization_id);
        resolvedUser = orgUsers?.[0];
      }
      if (!resolvedUser) {
        // ⚠️ Le bug Teva : token "valide" en apparence mais aucun user résolvable
        report.issues.push({ type: 'token_unresolvable', token_id: tk.id, org_id: tk.organization_id, email: tk.email });
        if (heal && tk.organization_id) {
          // Force la création du user passwordless via ensureOrgHasUser
          const newUser = await ensureOrgHasUser(db, tk.organization_id);
          if (newUser?.id) {
            await db.collection('access_tokens').updateOne({ id: tk.id }, { $set: { user_id: newUser.id, updated_at: new Date() } });
            report.healed.push({ type: 'token_user_created_and_linked', token_id: tk.id, user_id: newUser.id });
            userIds.add(newUser.id);
            users.push(newUser);
            if (!userByOrg.has(tk.organization_id)) userByOrg.set(tk.organization_id, []);
            userByOrg.get(tk.organization_id).push(newUser);
            if (newUser.email) userByEmail.set(newUser.email, newUser);
          }
        }
      }
    }
 ...
 * 🛡️ Race-safe : utilise findOneAndUpdate upsert pour éviter les duplications
 */
export async function ensureOrgHasUser(db, organizationId) {
  ...
  if (!org || org.deleted_at || org.archived_at) return null;
  // 1. Tente de trouver via organization_id
  let user = await db.collection('users').findOne({ organization_id: organizationId, role_code: 'exposant' });
  if (user) return user;
  // 2. Tente de trouver via email (et le relier à l'org si pas déjà fait)
  if (org.main_email) {
  ...
  // 3. Crée le user — race-safe via upsert sur l'email
  const exposantRole = await db.collection('roles').findOne({ code: 'exposant' });
  const email = (org.main_email || `${organizationId}@auto.aracom.pf`).toLowerCase().trim();
  const newUserId = uuid();
  const newUser = {
    id: newUserId,
    email,
  ...
  try {
    // Upsert sur email pour éviter les doublons (race condition)
    const result = await db.collection('users').findOneAndUpdate(
      { email },
      { $setOnInsert: newUser, $set: { organization_id: organizationId, updated_at: new Date() } },
      { upsert: true, returnDocument: 'after' }
    );
    return result.value || result || newUser;
  } catch (e) {
    // Si upsert échoue (très rare), re-fetch
