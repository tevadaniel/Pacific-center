...
export async function getWizardState(db, registration_id) {
  ...
  const stepStatus = {
    ...
    step2_booking: !!(reg.venue_id && reg.visit_slot_id),
    step3_animation: animSlots.length > 0 && animSlots.every(a => a.status !== 'annulé'),
    step4_docs_rdv: !!(valReq?.rdv_date || (docs.length >= 2 && reg.is_insurance_uploaded && reg.is_convention_signed)),
    step5_confirmed: reg.status === 'confirme',
  };
  const currentStep = stepStatus.step5_confirmed ? 5
    : stepStatus.step4_docs_rdv ? 5
    : stepStatus.step3_animation ? 4
    : stepStatus.step2_booking ? 3
