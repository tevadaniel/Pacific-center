      ...
      animation_grid: (() => {
        ...
        for (const d of WIZARD_CONFIG.DAYS) {
          ...
          grid.slots = grid.slots.map(s => {
            const occ = occupants.find(o => o.start_time === s.start && o.end_time === s.end);
            return occ
              ? { ...s, occupied: true, registration_id: occ.registration_id, organization_name: occ.organization_name, location_type: occ.location_type }
              : { ...s, occupied: false };
          });
          out[d.key] = grid;
        }
