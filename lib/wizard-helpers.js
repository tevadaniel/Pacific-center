...
export async function getWizardState(db, registration_id) {
  ...
  const stepStatus = {
    ...
    step5_docs_rdv: !!(valReq?.rdv_date || valReq?.status === 'rdv_fixe' || valReq?.status === 'en_attente' || (docs.length >= 2 && reg.is_insurance_uploaded && reg.is_convention_signed)),
    ...
    step4_docs_rdv: !!(valReq?.rdv_date || valReq?.status === 'rdv_fixe' || valReq?.status === 'en_attente' || (docs.length >= 2 && reg.is_insurance_uploaded && reg.is_convention_signed)),
