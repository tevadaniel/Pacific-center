 ...
 *
 * Le caller peut overrider en passant `testModeOverride` (booléen). Si défini,
 * cette valeur prend le pas sur la variable d'environnement (utilisé pour le
 * toggle DB-backed depuis l'UI ARACOM).
 *
 * @param {Object} opts
 ...
 * @param {boolean} [opts.testModeOverride] - if defined, overrides env MAIL_TEST_MODE
 * @param {string} [opts.redirectToOverride] - if defined, overrides env MAIL_REDIRECT_TO
 * @param {string[]} [opts.allowListOverride] - if defined, overrides env MAIL_ALLOWED_RECIPIENTS
 * @returns {Promise<{ok: boolean, messageId?: string, error?: string, redirected_from?: string}>}
 */
export async function sendMail({ to, subject, html, text, replyTo, testModeOverride, redirectToOverride, allowListOverride }) {
  if (!isSmtpConfigured()) {
  ...
  // 🛡️ TEST MODE GUARD — DB override > env var
  const envTestMode = String(process.env.MAIL_TEST_MODE || '').toLowerCase() === 'true';
  const testMode = testModeOverride !== undefined ? Boolean(testModeOverride) : envTestMode;
  const redirectTo = redirectToOverride || process.env.MAIL_REDIRECT_TO || 'tevageros@me.com';
  const envAllowList = (process.env.MAIL_ALLOWED_RECIPIENTS || '').split(',').map(s => s.trim().toLowerCase()).filter(Boolean);
  const allowList = (allowListOverride && allowListOverride.length) ? allowListOverride.map(s => String(s).toLowerCase()) : envAllowList;
  const originalTo = to;
