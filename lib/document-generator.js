...
function drawHeader(doc, title) {
  ...
  // Titre à droite — lineBreak:false pour éviter tout wrap qui écraserait le sous-titre
  doc.fillColor(C.gold).font('Helvetica-Bold').fontSize(13)
     .text(title.toUpperCase(), 230, 30, {
       width: 335, align: 'right', characterSpacing: 1.5, lineBreak: false, ellipsis: true,
     });
  doc.fillColor(C.beigeP).font('Helvetica').fontSize(7.5)
     .text('14 & 15 août 2026 · Polynésie française', 230, 56, {
       width: 335, align: 'right', lineBreak: false,
     });
}
...
export async function generateConventionPDF({ registration, organization, venue, animations = [], deposit = null }) {
  return new Promise((resolve, reject) => {
    try {
      ...
      kv(doc, 'Mode de règlement', deposit?.deposit_mode || '☐ Espèces  ☐ Chèque  ☐ Virement', 180, dy + 8, 200);
