  ...
  async _runOne(scenario) {
    ...
    try {
      ...
      if (String(e.message || '').startsWith('ABANDON_')) {
        ...
        // 🆕 SESSION 52d — Si abandon APRÈS création de la registration (step ≥ 2 = days enregistrés),
        // on cleanup pour éviter des candidatures fantômes (site+stand+jours mais 0 animation).
        if (step >= 2 && regId) {
        ...
        // 🆕 SESSION 52d — Cleanup aussi en cas d'erreur après création registration
        if (step >= 2 && regId) {
