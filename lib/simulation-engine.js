...
export class SimulationEngine {
  ...
  async start() {
    ...
    // 0) Charge dynamiquement la liste des venues réelles (évite hardcoding obsolète)
    try {
      const v = await fetch('/api/venues', { cache: 'no-store' });
      if (v.ok) {
        const arr = await v.json();
        const ids = (Array.isArray(arr) ? arr : []).map(x => x.id).filter(Boolean);
        if (ids.length) {
          VENUE_IDS.splice(0, VENUE_IDS.length, ...ids);
          this._emit({ type: 'state', message: `🏢 ${ids.length} sites détectés : ${ids.join(', ')}` });
        }
      }
