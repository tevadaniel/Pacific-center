...
export class SimulationEngine {
  ...
  async _runOne(scenario) {
    ...
    try {
      ...
      const freeStands = stands.filter(s => !s.assigned_registration_id && (s.status === 'libre' || !s.status));
      ...
      const r3 = await this._api('/wizard/stand', {
        method: 'POST',
        body: JSON.stringify({
          registration_id: regId,
          venue_id: scenario.venue_id,
          stand_code: chosenStand.code || chosenStand.stand_code,
        }),
      });
      this.stats.by_step.stand++;
      this._emit({ type: 'api', label: scenario.label, step: 'stand', path: '/wizard/stand', status: r3.status, elapsed: r3.elapsed, message: `🎪 Stand ${chosenStand.code || chosenStand.stand_code} attribué` });
