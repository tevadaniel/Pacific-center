...
function buildScenario(index, sessionId) {
  ...
  return {
    ...
    // 🆕 SESSION 52g.3 — Plus d'abandons fictifs. La simulation ne produit QUE des exposants
    // qui terminent réellement le tunnel (= 1 animation par jour). Si un blocage survient
    // (site plein, plus de créneaux), la registration est nettoyée et le scénario sauté.
  };
}
...
export class SimulationEngine {
  ...
  async _runOne(scenario) {
    ...
    try {
      ...
      // 🆕 SESSION 52g.3 — Plus aucun stand libre → on SKIP ce scénario (cleanup la reg orpheline).
      //   Pas de waitlist artificielle qui pollue le Cockpit avec des "(sans nom)".
      //   Reflète la réalité : un exposant qui voit "site plein" ferait la même chose (abandonne ou choisit ailleurs).
      if (!freeStands.length) {
        this._emit({ type: 'info', label: scenario.label, message: `↩ Site ${scenario.venue_id} plein — scénario sauté` });
        await this._cleanupAbandoned(regId, 'venue_full_skip');
      ...
      while (attempt < 4 && !r3) {
        ...
        try {
          ...
          // 🆕 SESSION 52g.3 — Conflit pending/waitlist (un autre worker a pris le stand entretemps)
          //   → on PIVOTE TOUJOURS vers un autre stand. Plus de waitlist artificielle.
          ...
          this._emit({ type: 'api', label: scenario.label, step: 'stand', path: '/wizard/stand', status: r3.status, elapsed: r3.elapsed, message: `🎪 Stand ${code} demandé${attempt > 0 ? ` (essai ${attempt + 1})` : ''}` });
        } catch (err) {
          attempt++;
      ...
      if (!r3) {
        // 🆕 SESSION 52g.3 — Tous les retries ont échoué → skip propre (cleanup)
        this._emit({ type: 'info', label: scenario.label, message: `↩ Aucun stand libre après 4 essais — scénario sauté` });
        await this._cleanupAbandoned(regId, 'no_stand_after_retries');
        this.stats.abandoned++;
        return;
      }
      ...
      // 🆕 SESSION 52g.3 — Retry sur conflit slot, JAMAIS de waitlist artificielle.
      //   Si tous les slots sont pris (réel ou conflit), on saute proprement le scénario.
      let r4 = null;
      for (let animAttempt = 0; animAttempt < 4 && !r4; animAttempt++) {
        ...
        const cleanAnims = animsToSend.map(a => {
          const { _backup_slots, ...clean } = a; // _backup_slots is intentionally discarded
          return clean;
        });
        try {
          const resp = await this._api('/wizard/animation', {
            method: 'POST',
            body: JSON.stringify({ registration_id: regId, animations: cleanAnims }),
          });
          // Conflit (HTTP 200 mais ok:false) → pivote sur slots de secours
          if (resp.data?.conflict === true) {
            let canPivot = true;
            ...
            if (!canPivot) {
              this._emit({ type: 'info', label: scenario.label, message: `↩ Plus de créneaux libres — scénario sauté` });
              await this._cleanupAbandoned(regId, 'no_backup_slot');
              this.stats.abandoned++;
              return;
            }
          ...
          // Conflit DB → pivote slot, sinon throw
      ...
      if (!r4) {
        this._emit({ type: 'info', label: scenario.label, message: `↩ Aucun créneau d'animation après 4 essais — scénario sauté` });
      ...
      if (this.state !== 'stopped') {
        ...
        // 🆕 SESSION 52g.3 — Cleanup TOUJOURS si une reg a été créée. Pas de fantômes en DB.
        if (regId) {
