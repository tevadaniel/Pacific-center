  ...
  async _runOne(scenario) {
    ...
    try {
      ...
      for (const dayKey of scenario.attending_days) {
        ...
        const freeSlot = slots.find(s => !s.occupied && !occupiedKeys.has(`${dayKey}-${s.start}`));
        if (!freeSlot) {
      ...
      // 🐛 FIX SESSION 47.5 — Backend attend `animations` (pas `slots`) + un slot par jour de présence + slot_type obligatoire
      const r4 = await this._api('/wizard/animation', {
        method: 'POST',
        body: JSON.stringify({
          registration_id: regId,
          animations: animsToSend,
        }),
      });
      this.stats.by_step.animation++;
      this._emit({ type: 'api', label: scenario.label, step: 'animation', path: '/wizard/animation', status: r4.status, elapsed: r4.elapsed, message: `🎭 ${animsToSend.length} animation${animsToSend.length > 1 ? 's' : ''} créée${animsToSend.length > 1 ? 's' : ''} (${animsToSend.map(a => a.day_label).join('+')})` });
