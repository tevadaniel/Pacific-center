...
// Fallback si l'API venues échoue. Sera écrasé dynamiquement par fetch('/api/venues').
let VENUE_IDS = ['venue-faaa', 'venue-pun', 'venue-aru', 'venue-tar'];
...
function buildScenario(index, sessionId) {
  ...
  return {
    ...
    representatives_count: 1 + Math.floor(Math.random() * 2), // 1 ou 2 (backend exige 1-2)
...
export class SimulationEngine {
  ...
  async _api(path, opts = {}) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        // best-effort calls don't pollute stats.errors
        if (!opts.bestEffort) {
  ...
  /**
   * 🆕 SESSION 52d — Cleanup d'une registration abandonnée mid-flow.
   * Évite de laisser des candidatures à moitié remplies (site+stand+jours mais 0 ou 1 animation),
   * qui pollueraient la base et fausseraient les statistiques.
   * Best-effort : ne lève pas d'exception (sinon l'engine plante).
   */
  async _cleanupAbandoned(regId, reason) {
    if (!regId) return;
    try {
      await this._api('/admin/simulation/abandon-cleanup', {
        method: 'POST',
        body: JSON.stringify({ registration_id: regId }),
        bestEffort: true,
      });
      this._emit({ type: 'info', message: `🧹 Reg ${regId.slice(0, 8)}… nettoyée (${reason || 'abandon'})` });
    } catch (e) {
      // Best-effort : on log juste, sans arrêter
      this._emit({ type: 'warn', message: `⚠ Cleanup reg ${regId.slice(0, 8)}… échoué : ${e.message}` });
    }
  }
  ...
  async _runOne(scenario) {
    ...
    try {
      ...
      while (attempt < 4 && !r3) {
        ...
        triedCodes.add(code);
        try {
          let resp = await this._api('/wizard/stand', {
            method: 'POST',
            body: JSON.stringify({
              registration_id: regId,
              venue_id: scenario.venue_id,
          ...
          // 🐛 FIX SESSION 47.13 — Conflit pending/waitlist : HTTP 200 mais {ok:false, conflict:true}
          // → Accepter la liste d'attente en relançant avec force_waitlist (~50% du temps, sinon pivote)
          if (resp.data?.conflict === true) {
            const acceptWaitlist = Math.random() < 0.5; // 50% acceptent la waitlist
            if (acceptWaitlist) {
              resp = await this._api('/wizard/stand', {
                method: 'POST',
                body: JSON.stringify({
                  registration_id: regId,
                  venue_id: scenario.venue_id,
              ...
              this._emit({ type: 'waitlist', label: scenario.label, message: `📋 Stand ${code} — accepte la liste d'attente (position #${resp.data?.waitlist_position || '?'})` });
            } else {
              // Pivote vers un autre stand
              attempt++;
              this._emit({ type: 'warn', label: scenario.label, message: `↻ Stand ${code} en attente, pivote vers un autre` });
          ...
          this._emit({ type: 'api', label: scenario.label, step: 'stand', path: '/wizard/stand', status: r3.status, elapsed: r3.elapsed, message: `🎪 Stand ${code} ${r3.data?.request_status === 'waitlist' ? 'en liste d\'attente' : 'demandé'}${attempt > 0 ? ` (essai ${attempt + 1})` : ''}` });
        } catch (err) {
          attempt++;
      ...
      // STEP 4 — Animation (créneau dynamique) — 1 animation OBLIGATOIRE par jour de présence
      await sleep(rand(500, 2000)); await this._waitIfPaused();
      if (this.state === 'stopped') return;
      step = 4;
      // Lit la disponibilité pour trouver des créneaux libres
      ...
      for (const dayKey of scenario.attending_days) {
        const grid = venue?.animation_grid?.[dayKey];
        const slots = (grid?.slots || []).filter(s => !s.occupied && !occupiedKeys.has(`${dayKey}-${s.start}`));
        if (!slots.length) {
          this._emit({ type: 'warn', label: scenario.label, message: `⚠ Aucun créneau d'animation libre (${dayKey}) — abandon` });
          this.stats.abandoned++;
          await this._cleanupAbandoned(regId, `no_slot_${dayKey}`);
          return;
        }
        // 🐛 FIX SESSION 47.12 — Sélectionne un slot ALÉATOIRE (pas le premier) pour réduire les collisions concurrentes
        const freeSlot = pick(slots);
        occupiedKeys.add(`${dayKey}-${freeSlot.start}`);
        animsToSend.push({
          day_label: dayKey,
          ...
          // Liste de slots de secours pour le retry côté engine
          _backup_slots: slots.filter(s => s.start !== freeSlot.start).slice(0, 5).map(s => ({ start: s.start, end: s.end })),
        });
      }

      // 🐛 FIX SESSION 47.12 — Retry automatique en cas de conflit (un autre worker a pris le slot)
      // 🐛 FIX SESSION 47.13 — Gère aussi la nouvelle réponse {ok:false, conflict:true} (HTTP 200)
      let r4 = null;
      for (let animAttempt = 0; animAttempt < 4 && !r4; animAttempt++) {
        // Strip les champs internes _backup_slots avant envoi
        const cleanAnims = animsToSend.map(a => {
          // eslint-disable-next-line no-unused-vars
          const { _backup_slots, ...clean } = a;
          return clean;
        });
        try {
          let resp = await this._api('/wizard/animation', {
            method: 'POST',
            body: JSON.stringify({ registration_id: regId, animations: cleanAnims }),
          });
          // 🐛 FIX SESSION 47.13 — Conflit pending/waitlist (HTTP 200 mais {ok:false, conflict:true})
          //   → soit accepter la waitlist (50%), soit pivoter
          if (resp.data?.conflict === true) {
            const acceptWaitlist = Math.random() < 0.5;
            if (acceptWaitlist) {
              resp = await this._api('/wizard/animation', {
                method: 'POST',
                body: JSON.stringify({ registration_id: regId, animations: cleanAnims, force_waitlist: true }),
              });
              this._emit({ type: 'waitlist', label: scenario.label, message: `📋 Animation — accepte la liste d'attente (${(resp.data?.waitlist_count) || 0}/${cleanAnims.length} slots)` });
              r4 = resp;
              this.stats.by_step.animation++;
              this._emit({ type: 'api', label: scenario.label, step: 'animation', path: '/wizard/animation', status: r4.status, elapsed: r4.elapsed, message: `🎭 ${cleanAnims.length} animation${cleanAnims.length > 1 ? 's' : ''} demandée${cleanAnims.length > 1 ? 's' : ''} (waitlist activée)` });
            } else {
              // Pivote chaque animation sur un slot de secours
              let canPivot = true;
              for (const a of animsToSend) {
                if (a._backup_slots && a._backup_slots.length > 0) {
                  const next = a._backup_slots.shift();
                  a.start_time = next.start;
                  a.end_time = next.end;
                } else { canPivot = false; break; }
              }
              if (!canPivot) {
                this._emit({ type: 'warn', label: scenario.label, message: `⚠ Plus de créneaux de secours pour ${scenario.attending_days.join('/')} — abandon` });
                this.stats.abandoned++;
                await this._cleanupAbandoned(regId, 'no_backup_pending');
                return;
              }
              this._emit({ type: 'warn', label: scenario.label, message: `↻ Conflit animation, pivote vers slot suivant (essai ${animAttempt + 2})` });
            ...
            r4 = resp;
            this.stats.by_step.animation++;
            this._emit({ type: 'api', label: scenario.label, step: 'animation', path: '/wizard/animation', status: r4.status, elapsed: r4.elapsed, message: `🎭 ${cleanAnims.length} animation${cleanAnims.length > 1 ? 's' : ''} créée${cleanAnims.length > 1 ? 's' : ''} (${cleanAnims.map(a => `${a.day_label} ${a.start_time}`).join(', ')})` });
          }
        } catch (e) {
          // Si conflit → pivote chaque animation sur un slot de secours
          if (/déjà pris|conflit/i.test(e.message) && animAttempt < 3) {
            let canPivot = true;
            for (const a of animsToSend) {
              if (a._backup_slots && a._backup_slots.length > 0) {
                const next = a._backup_slots.shift();
            ...
            if (!canPivot) {
              this._emit({ type: 'warn', label: scenario.label, message: `⚠ Plus de créneaux de secours pour ${scenario.attending_days.join('/')} — abandon` });
              this.stats.abandoned++;
              await this._cleanupAbandoned(regId, 'no_backup_conflict');
              return;
            }
            this._emit({ type: 'warn', label: scenario.label, message: `↻ Conflit animation, pivote vers slot suivant (essai ${animAttempt + 2})` });
            await sleep(rand(150, 400));
      ...
      if (!r4) {
        await this._cleanupAbandoned(regId, 'no_slot_after_4_retries');
        throw new Error('Aucun créneau d\'animation après 4 essais');
      }

      if (scenario.abandon_step === 5) throw new Error('ABANDON_STEP_5');

      // STEP 5 — Finalize avec paramètres caution (Session 47.4)
      await sleep(rand(500, 2000)); await this._waitIfPaused();
      if (this.state === 'stopped') return;
      step = 5;
      // Génère une date de dépôt caution aléatoire entre demain et J-1 de l'événement (14/08/2026)
      const depositDate = new Date();
      depositDate.setDate(depositDate.getDate() + 7 + Math.floor(Math.random() * 30));
      const depositDateStr = depositDate.toISOString().slice(0, 10);
      const depositTime = `${String(9 + Math.floor(Math.random() * 8)).padStart(2, '0')}:${pick(['00', '15', '30', '45'])}`;
      const paymentMethod = 'cheque'; // 🆕 SESSION 48w — Caution = chèque uniquement
      const r5 = await this._api('/wizard/finalize', {
        ...
        body: JSON.stringify({
          ...
          caution_deposit_date: depositDateStr,
          caution_deposit_time: depositTime,
          bring_documents_to_rdv: true,
        }),
      });
      this.stats.by_step.finalize++;
      this._emit({ type: 'success', label: scenario.label, step: 'finalize', path: '/wizard/finalize', status: r5.status, elapsed: r5.elapsed, message: `✅ Parcours terminé pour ${scenario.name} (caution ${paymentMethod}, RDV ${depositDateStr})` });
      ...
      if (String(e.message || '').startsWith('ABANDON_')) {
        ...
        // 🆕 SESSION 52g — Cleanup AGRESSIF : dès qu'une reg existe (step ≥ 0 = self-register fait),
        // on la nettoie. Sinon on laisse des emails fantômes en DB.
        if (regId) {
          await this._cleanupAbandoned(regId, `abandon_step_${step + 1}`);
        }
      } else if (this.state !== 'stopped') {
        this.stats.failed++;
        this._emit({ type: 'error', label: scenario.label, step, message: `❌ Erreur étape ${step + 1} : ${e.message}` });
        // 🆕 SESSION 52g — Idem : cleanup dès qu'une reg a été créée
        if (regId) {
          await this._cleanupAbandoned(regId, `error_step_${step + 1}`);
        }
      }
    } finally {
      this.stats.in_progress--;
  ...
  async start() {
    ...
    // 0) Charge dynamiquement la liste des venues ACTIVES (only_active=1 → exclut Mahina/Moorea désactivés)
    // 🆕 SESSION 52f — Strictement les sites activés pour respecter la logique plateforme
    try {
      const v = await fetch('/api/venues?only_active=1', { cache: 'no-store' });
      if (v.ok) {
        const arr = await v.json();
        const ids = (Array.isArray(arr) ? arr : []).map(x => x.id).filter(Boolean);
        if (ids.length) {
          VENUE_IDS.splice(0, VENUE_IDS.length, ...ids);
          this._emit({ type: 'state', message: `🏢 ${ids.length} sites ACTIFS détectés : ${ids.join(', ')}` });
        }
      }
    } catch (e) { this._emit({ type: 'warn', message: `⚠ Impossible de charger les venues (utilise fallback) : ${e.message}` }); }
    ...
    // 🆕 SESSION 52g — CLEANUP FINAL : supprime toutes les regs de simulation INCOMPLÈTES
    // (status != a_confirmer/confirme/verrouille), pour ne pas polluer le Cockpit.
    // Les waitlists, brouillons abandonnés et erreurs sont nettoyés ici.
    try {
      const r = await fetch('/api/admin/simulation/cleanup-incomplete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...this.adminHeaders },
        body: '{}',
      });
      ...
      if (r.ok && data?.deleted) {
        ...
        this._emit({ type: 'state', message: `🧹 Cleanup final : ${d.registrations} regs + ${d.organizations} orgs supprimées (incomplètes)` });
      }
    } catch (e) {
      this._emit({ type: 'warn', message: `⚠ Cleanup final échoué : ${e.message}` });
    }
