...
// x, y : centre du stand
function stand(code, x, y, w = 46, h = 28) {
  const rx = Math.round(w / 2);
  return `<rect x="${x - w/2}" y="${y - h/2}" width="${w}" height="${h}" rx="${rx}" fill="${CYAN}"/><text x="${x}" y="${y + 4}" text-anchor="middle" font-size="11" font-weight="800" fill="#fff" class="snum">${code}</text><text x="${x}" y="${y + 4}" text-anchor="middle" font-size="9" font-weight="700" fill="#fff" class="snom" style="display:none"></text>`;
}

// Génère un kiosque (ovale noir bordure cyan)
function kiosque(x, y, w = 90, h = 28) {
  const rx = Math.round(h / 2);
  return `<rect x="${x - w/2}" y="${y - h/2}" width="${w}" height="${h}" rx="${rx}" fill="${BLACK}" stroke="${CYAN}" stroke-width="2"/><text x="${x}" y="${y + 4}" text-anchor="middle" font-size="13" font-weight="700" fill="#fff">Kiosque</text>`;
}

// Génère la zone DEMONSTRATION (vert avec bordure cyan)
function demo(x, y, w = 130, h = 48) {
...
function commerce(x, y, w, h, label = 'Commerce') {
  return `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${BLACK}"/><text x="${x + w/2}" y="${y + h/2 + 9}" text-anchor="middle" font-size="26" font-weight="800" fill="#fff">${label}</text>`;
}
