/**
 * 📥 IMPORT FUSION 2026 — Parse Excel "Forum BDD Rentrée 2026 FUSION" → seed clean DB.
 *
 * Source : /app/data-imports/forum-fusion-2026.xlsx (uploaded by admin)
 * Cible  : Wipe + recreate organizations + registrations (status='a_confirmer')
 *          UNIQUEMENT la feuille "📋 BASE EXPOSANTS" (72 historiques)
 *          La feuille "📞 PROSPECTION 2026" est IGNORÉE (choix admin SESSION 53.4).
 *
...
const EXCEL_PATH = path.join(process.cwd(), 'data-imports', 'forum-fusion-2026.xlsx');
const SHEET_NAME = '📋 BASE EXPOSANTS';
...
export function previewImport() {
  ...
  return {
    ...
    sample: exposants.slice(0, 5).map(e => ({ name: e.name, email: e.email, contact: e.contactName, fidelity: e.fidelity, site: e.sitePrincipal })),
  };
}
...
export async function executeImport(db, { dryRun = false, actorUserId = 'u-admin' } = {}) {
  ...
  for (const e of exposants) {
    let orgId = `org-fusion-${slugify(e.name)}`;
    // Eviter collision si 2 noms ont le même slug
    let suffix = 1;
    while (usedIds.has(orgId)) {
      suffix += 1;
      orgId = `org-fusion-${slugify(e.name)}-${suffix}`;
    }
    usedIds.add(orgId);
    ...
    await db.collection('organizations').insertOne({
      ...
      // Historique de participation embarqué pour le mailing/ciblage
    ...
    // Contact principal (uniquement si on a un nom)
    if (e.contactName) {
    ...
    // Registration 2026 — site/stand vide par choix admin (historique uniquement)
    const regId = `reg-fusion-${slugify(e.name)}`;
