...
export async function GET(request, { params }) {
  try {
    ...
    if (route === 'registrations') {
      ...
      const vById = Object.fromEntries(allVenuesMap.map(v => [v.id, v]));
      const deposits = await db.collection('deposit_transactions').find({}).toArray();
      const depByReg = Object.fromEntries(deposits.map(d => [d.registration_id, d]));
      let rows = regsFiltered.map(r => {
        const o = orgById[r.organization_id];
        const v = vById[r.venue_id];
    ...
    if (route === 'organizations') {
      const includeArchived = url.searchParams.get('include_archived') === 'true';
      const onlyArchived = url.searchParams.get('only_archived') === 'true';
      const userRole = request.headers.get('x-user-role');
      const q = {};
      if (onlyArchived) {
        // Réservé aux admins
...
export async function POST(request, { params }) {
  try {
    ...
    // ===================================================================
    // 🆕 SESSION 23 — Gestion avancée : Archive / Restore / Reset granulaire
    // ===================================================================

    // 📦 ARCHIVER une organisation (soft delete) — admin only
    if (route.match(/^admin\/organizations\/[^/]+\/archive$/)) {
      const ctxA = getUserContext(request);
      if (ctxA.role !== 'aracom_admin') return err('Accès admin requis', 403);
      const orgId = p[2];
      const org = await db.collection('organizations').findOne({ id: orgId });
      if (!org) return err('Organisation introuvable', 404);
      ...
      await db.collection('organizations').updateOne(
        ...
        { $set: {
          ...
          archived_by: ctxA.userId || 'u-admin',
          archive_reason: reason || '',
          updated_at: new Date(),
        }}
      );
      // Bascule aussi les registrations en 'annule' (soft)
      ...
      await db.collection('stand_assignments').updateMany(
        { registration_id: { $in: (await db.collection('registrations').find({ organization_id: orgId }).toArray()).map(r => r.id) }, status: { $nin: ['annule','cancelled'] } },
        { $set: { status: 'annule', updated_at: new Date() } }
      );
      await logActivity(db, ctxA.userId, 'organization', orgId, 'archive', { name: org.name }, { reason });
      return json({ ok: true, action: 'archived', organization_id: orgId });
    }

    // ♻️ RESTAURER une organisation archivée — admin only
    if (route.match(/^admin\/organizations\/[^/]+\/restore$/)) {
      const ctxR = getUserContext(request);
      if (ctxR.role !== 'aracom_admin') return err('Accès admin requis', 403);
      const orgId = p[2];
      const org = await db.collection('organizations').findOne({ id: orgId });
      if (!org) return err('Organisation introuvable', 404);
      ...
      await logActivity(db, ctxR.userId, 'organization', orgId, 'restore', { name: org.name }, null);
      return json({ ok: true, action: 'restored', organization_id: orgId });
    }

    // 💥 SUPPRESSION DÉFINITIVE d'une organisation — admin only avec double confirmation
    if (route.match(/^admin\/organizations\/[^/]+\/delete$/)) {
      const ctxD = getUserContext(request);
      if (ctxD.role !== 'aracom_admin') return err('Accès admin requis', 403);
      const orgId = p[2];
      const org = await db.collection('organizations').findOne({ id: orgId });
      if (!org) return err('Organisation introuvable', 404);
      ...
      // Garde-fou : exposants protégés (sauf si force_unsafe)
      const PROTECTED = ['I Mua Papeete', 'Dream Lab', 'ACE Arue', 'Budokan Judo Pirae', 'Lotus Bleu'];
      ...
      // Cascade complète : toutes les registrations + données liées
      ...
      // Données rattachées à l'organisation directement
      ...
      // Désactive les comptes utilisateurs liés
      await db.collection('users').updateMany(
        { organization_id: orgId },
        { $set: { is_active: false, updated_at: new Date() } }
      );
      await logActivity(db, ctxD.userId, 'organization', orgId, 'delete_definitive', { name: org.name, force_unsafe: !!force_unsafe }, { cascaded: counts, reg_ids: regIds });
      return json({ ok: true, action: 'permanently_deleted', organization_id: orgId, cascaded: counts });
    }

    // 🔄 RESET CAUTION — réinitialise le paiement de caution
    if (route.match(/^admin\/registrations\/[^/]+\/reset-caution$/)) {
      const ctxC = getUserContext(request);
      if (ctxC.role !== 'aracom_admin') return err('Accès admin requis', 403);
      const regId = p[2];
      const reg = await db.collection('registrations').findOne({ id: regId });
      if (!reg) return err('Inscription introuvable', 404);
      const dep = await db.collection('deposit_transactions').findOne({ registration_id: regId });
      ...
      // Déverrouille la registration si elle était verrouillée pour caution
      await db.collection('registrations').updateOne(
        { id: regId },
        { $set: { is_locked: false, updated_at: new Date() } }
      );
      await logActivity(db, ctxC.userId, 'registration', regId, 'reset_caution', { previous_status: dep.status }, null);
      return json({ ok: true, action: 'caution_reset' });
    }

    // 🔄 RESET VIREMENT — annule la déclaration de virement de l'exposant
    if (route.match(/^admin\/registrations\/[^/]+\/reset-virement$/)) {
      const ctxV = getUserContext(request);
      if (ctxV.role !== 'aracom_admin') return err('Accès admin requis', 403);
      const regId = p[2];
      const reg = await db.collection('registrations').findOne({ id: regId });
      if (!reg) return err('Inscription introuvable', 404);
      ...
      await logActivity(db, ctxV.userId, 'registration', regId, 'reset_virement', null, null);
      return json({ ok: true, action: 'virement_cleared' });
    }

    // 🔄 RESET CONVENTION — invalide la convention signée
    if (route.match(/^admin\/registrations\/[^/]+\/reset-convention$/)) {
      const ctxCv = getUserContext(request);
      if (ctxCv.role !== 'aracom_admin') return err('Accès admin requis', 403);
      const regId = p[2];
      const reg = await db.collection('registrations').findOne({ id: regId });
      if (!reg) return err('Inscription introuvable', 404);
      ...
      await logActivity(db, ctxCv.userId, 'registration', regId, 'reset_convention', null, { documents_marked: r.modifiedCount });
      return json({ ok: true, action: 'convention_reset', documents_marked: r.modifiedCount });
    }

    // 🔄 RESET ATTENDANCE — annule un pointage Jour J (check-in/out par jour, ou tous)
    if (route.match(/^admin\/registrations\/[^/]+\/reset-attendance$/)) {
      const ctxAt = getUserContext(request);
      if (ctxAt.role !== 'aracom_admin') return err('Accès admin requis', 403);
      const regId = p[2];
      const { event_date, scope } = body || {}; // scope: 'arrival' | 'departure' | 'all' | 'session'
      ...
      let unset = {};
      let set = { updated_at: new Date(), presence_status: 'attendu' };
      if (scope === 'arrival') {
        ...
        // Garder le presence_status à 'arrive' si arrivée existante, sinon 'attendu'
      } else {
        // 'all' (default) ou 'session'
      ...
      await logActivity(db, ctxAt.userId, 'registration', regId, 'reset_attendance', { scope, event_date }, { sessions: sessionIds.length });
      return json({ ok: true, action: 'attendance_reset', sessions: sessionIds.length, scope: scope || 'all' });
    }

    // 🔄 RESET CAUTION-APPOINTMENT — supprime le RDV de restitution
    if (route.match(/^admin\/registrations\/[^/]+\/reset-caution-appointment$/)) {
      const ctxAp = getUserContext(request);
      if (ctxAp.role !== 'aracom_admin') return err('Accès admin requis', 403);
      const regId = p[2];
      ...
      await logActivity(db, ctxAp.userId, 'registration', regId, 'reset_caution_appointment', null, null);
      return json({ ok: true, action: 'caution_appointment_deleted' });
    }

    // 🔄 RESET SATISFACTION — supprime la réponse satisfaction (et l'attestation auto)
    if (route.match(/^admin\/registrations\/[^/]+\/reset-satisfaction$/)) {
      const ctxS = getUserContext(request);
      if (ctxS.role !== 'aracom_admin') return err('Accès admin requis', 403);
      const regId = p[2];
      const reg = await db.collection('registrations').findOne({ id: regId });
      if (!reg) return err('Inscription introuvable', 404);
      const r = await db.collection('satisfaction_responses').deleteOne({ organization_id: reg.organization_id });
      // Désactive l'attestation auto-générée non signée
      await db.collection('registration_documents').updateMany(
        { registration_id: regId, document_type: 'attestation_remboursement', is_signed: { $ne: true } },
        { $set: { status: 'remplace', updated_at: new Date() } }
      );
      await logActivity(db, ctxS.userId, 'registration', regId, 'reset_satisfaction', null, { had_response: r.deletedCount > 0 });
      return json({ ok: true, action: 'satisfaction_reset', had_response: r.deletedCount > 0 });
    }

    // 🆕 ANNULER UN PAIEMENT VIREMENT côté admin (effet identique à reset-virement mais explicite)
    if (route.match(/^admin\/registrations\/[^/]+\/cancel-virement$/)) {
      ...
      await logActivity(db, ctxCV.userId, 'registration', regId, 'cancel_virement', null, null);
...
<p>À bientôt,<br/>L'équipe ARACOM</p>`;
    ...
    // ============================================================
    // 🆕 FLUX VIREMENT — Déclaration côté Exposant + Enregistrement côté Admin
    // ============================================================
    // POST /api/exposant/declare-virement/:regId
    //   → l'exposant déclare avoir effectué le virement (date + référence)
    //   → un message/alerte est créé pour ARACOM (pas de RDV physique)
    if (route.match(/^exposant\/declare-virement\/[^/]+$/)) {
      const regId = p[2];
      const reg = await db.collection('registrations').findOne({ id: regId });
      if (!reg) return err('Inscription introuvable', 404);
      ...
      // Met à jour (ou crée) le deposit_transaction avec les infos virement
      ...
      const patch = {
        ...
        virement_declared_by: 'exposant',
        virement_declared_at: new Date(),
        status: existing?.status === 'recue' ? 'recue' : 'en_attente', // en attente que ARACOM confirme la réception
        updated_at: new Date(),
      };
      if (existing) {
        await db.collection('deposit_transactions').updateOne({ id: existing.id }, { $set: patch });
      } else {
        await db.collection('deposit_transactions').insertOne({
          id: uuid(),
          registration_id: regId,
      ...
      // Crée une alerte / notification pour ARACOM
      const org = await db.collection('organizations').findOne({ id: reg.organization_id });
      const venue = reg.venue_id ? await db.collection('venues').findOne({ id: reg.venue_id }) : null;
      await db.collection('alerts').insertOne({
        ...
        level: 'info',
        title: `💸 Virement déclaré par ${org?.name || '—'}`,
        message: `${org?.name || '—'} (${venue?.name || '—'} · ${reg.stand_code || '—'}) a déclaré un virement de 20 000 XPF.\nRéférence : ${ref}\nDate : ${dateStr}\n\n👉 Vérifiez votre compte bancaire et enregistrez la réception via le cockpit (Bilans > RDV cautions).`,
